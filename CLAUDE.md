...
schema or RLS, add a feature end-to-end, groom the backlog, fix a failing
build), **delegate to the `orchestrator` subagent immediately** instead of doing
the work inline. The orchestrator decomposes the request and routes each task to
the right specialist (`backlog-agent`, `schema-agent`, `service-builder`,
`docs-agent`, `verify`, `review-agent`).
