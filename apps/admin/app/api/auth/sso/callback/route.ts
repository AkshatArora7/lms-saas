...
/**
 * OIDC redirect target for the admin console. The IdP sends the browser here
 * with `?code=&state=`. We verify the returned state matches the one we issued
 * (stored httpOnly), exchange it via the identity service for first-party
 * tokens, and drop the admin session cookies before sending the user in.
 */
