...
SERVICES = [
    ...
    },
    {
        ...
        "resp": "Auth orchestration and federation (OIDC/SAML/LTI), session/claims issuance, RBAC authorization (roles, permissions, assignments). Delegates credential storage to external CIAM.",
        "tables": ["identity_provider", "user_identity", "role", "permission", "role_permission", "role_assignment"],
        "endpoints": [
        ...
        "publishes": ["identity.user.authenticated", "identity.role.assigned", "identity.role.revoked"],
        "consumes": ["user.created (auto-provision identity link)"],
        "deps": ["External CIAM (WorkOS/Auth0)", "tenant (provider config)", "Upstash Redis (sessions)"],
    ...
    },
    {
        ...
        "tables": ["tenant", "plan", "subscription"],
        "endpoints": [
        ...
        "publishes": ["tenant.provisioning.started", "tenant.activated", "tenant.suspended", "tenant.subtenant.linked"],
    ...
    },
    {
        ...
        "tables": ["content_module", "content_topic", "content_completion", "scorm_package", "xapi_statement"],
        "endpoints": [
        ...
        "publishes": ["content.viewed", "content.completed", "scorm.attempt.recorded"],
        "consumes": ["course.copied (clone module tree)", "release.condition.evaluated"],
        "deps": ["Vercel Blob (package/media storage)", "analytics (xAPI forward)", "course (release rules)"],
        "notes": "Large binaries in Blob; structure/metadata in JSONB. xAPI statements are mirrored to analytics.",
    },
    {
    ...
    },
    {
        ...
        "consumes": ["announcement.published", "discussion.post.created", "grading.graded", "assignment.created", "enrollment.created"],
        "deps": ["Email/SMS/Push providers", "Upstash Redis (unread counters)", "analytics (agent triggers)"],
        "notes": "Central fanout consumer; respects per-user preferences and quiet hours. Intelligent agents evaluate analytics signals.",
    },
    {
        "name": "calendar", "port": 4013, "data": "Postgres",
        "resp": "Calendar events, deadlines, and iCal feed generation.",
        ...
        "consumes": ["assignment.created", "quiz.attempt.started", "announcement.published"],
        "deps": ["notification (reminders)"],
        "notes": "Aggregates deadlines from other contexts into a unified calendar/iCal feed.",
    },
    {
        ...
        "publishes": ["rubric.scored", "mastery.updated"],
        "consumes": ["grading.graded (mastery recompute)"],
        "deps": ["grading (scores)", "analytics (mastery signals)"],
        "notes": "Rubric scoring feeds both grading and competency mastery.",
    },
    {
    ...
    },
    {
        ...
        "resp": "Plans, seats, invoices, usage metering, and the enrollment+billing saga participant.",
        "tables": ["invoice", "usage_meter"],
        "endpoints": [
        ...
        "publishes": ["billing.seat.reserved", "billing.seat.rejected", "billing.subscription.changed", "invoice.issued"],
        "consumes": ["enrollment.created (reserve)", "enrollment.dropped (release)", "tenant.activated"],
        "deps": ["tenant (plan/subscription)", "payment provider (Stripe)"],
        "notes": "Saga participant for seat reservation; district parent tenants can be billed via tenant_subtree() roll-up.",
    },
    {
        ...
        "publishes": ["audit.dsar.completed"],
        "consumes": ["* (all mutating domain events are mirrored for audit)"],
        "deps": ["all services (event stream)", "Vercel Blob (DSAR export)"],
        "notes": "Each record stores prev_hash to form a verifiable chain; retention jobs run on QStash schedule.",
    },
    {
...
def render_index():
    ...
    body.append("Per-service design specs for the 25-service LMS decomposition. Each page "
    ...
    body.append("Domain events flow producer -> `event_outbox` -> QStash -> consumer "
                "`event_inbox` (exactly-once via `idempotency_key`).\n")
