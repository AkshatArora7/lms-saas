...
SERVICES = [
    ...
    },
    {
        ...
        "notes": "Control-plane; `tenant` is NOT in the RLS tenant_tables loop. Provisioning is a saga with compensation (delete branch on failure). Offboarding orchestrates per-service export/purge behind ports; per-service admin export/erasure endpoints are the contract (unverified services surface as failed, never silent).",
    },
    {
    ...
    },
    {
        ...
        "tables": ["content_module", "content_topic", "content_completion", "page", "page_version", "release_condition", "scorm_package", "xapi_statement"],
        "endpoints": [
    ...
    },
    {
        ...
        "resp": "Lecture-video bounded context: signed direct-to-Blob uploads (tenant-namespaced), an injectable async transcode->caption pipeline that drives the video_asset lifecycle (uploaded->transcoding->ready), and URL-based adaptive playback (renditions + captions served from Blob/CDN, never proxied). Tenant-isolated by Postgres RLS.",
        "tables": ["video_asset"],
        "endpoints": [
        ...
        "publishes": [],
        "consumes": [],
        "deps": ["Vercel Blob (signed upload, DevBlobSigner default; production signer is a follow-up)", "FFmpeg worker (container host; real transcoder is a follow-up behind the Transcoder seam)", "ASR provider (real auto-captioner is a follow-up behind the Captioner seam)"],
