...
SERVICES = [
    ...
    },
    {
        ...
        "notes": "Read-heavy; backed by materialised membership views. OneRoster `users`/`orgs` map here. COPPA: age stored as a coarse band (not DOB); under-13 data handling is gated on verifiable parental consent (see docs/compliance/coppa-data-flows.md). Guardian links are read-only and consent-gated: `/guardians/authorize` re-derives the consent decision live, so a consent revoke denies access immediately (no separate guardian write path). `GET /guardians/:guardianId/children/authorized` (#190) is the consent-filtered, batch authorized-children read other services depend on (e.g. the attendance guardian-scoped view): it filters links to status='active' then re-derives each child's gating consent (directory_information) live, returning only the children currently permitted — unlike `/guardians/:guardianId/students`, which returns all statuses and is NOT consent-filtered.",
    },
    {
    ...
    },
    {
        ...
        "resp": "AI study assistant: embeds a course's content into pgvector and answers student questions via Groq RAG with citations, grounded ONLY in retrieved chunks. Tenant-isolated by Postgres RLS.",
        ...
        "publishes": [],
        "consumes": [],
        "deps": ["Groq (LLM, GROQ_API_KEY, optional)", "pgvector", "content (content_topic.body, direct RLS-scoped read)"],
