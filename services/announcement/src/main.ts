 ...
 * Course/org-unit announcements with scheduled publish and expiry. Publishing
 * emits announcement.published, which the notification service fans out to
 * recipients (in-app + email) per their preferences. Sits behind the gateway,
 * which authenticates requests and forwards the resolved tenant as
 * `x-tenant-id`.
 *
...
export interface BuildAppOptions {
  config?: AppConfig;
  store?: AnnouncementRouteDeps["store"];
  resolveTenant?: AnnouncementRouteDeps["resolveTenant"];
}
...
/**
 * Local dev convenience: `ANNOUNCEMENT_STORE=memory` runs the service against an
 * in-memory store seeded with a demo announcement so the gateway ->
 * announcement path works end-to-end without a Postgres database. Production
 * leaves this unset.
 */
const useMemoryStore = process.env.ANNOUNCEMENT_STORE === "memory";
