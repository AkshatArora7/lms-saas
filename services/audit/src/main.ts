/**
 * audit service.
 * Immutable, tamper-evident audit log (per-tenant hash chain over audit_log's
 * prev_hash/row_hash). Sits behind the gateway, which authenticates requests
 * and forwards the resolved tenant as `x-tenant-id`. Append-only; a verification
 * job re-hashes the chain to detect tampering.
 *
...
import { registerAuditRoutes, type AuditRouteDeps } from "./routes.js";
...
export interface BuildAppOptions {
  config?: AppConfig;
  store?: AuditRouteDeps["store"];
  resolveTenant?: AuditRouteDeps["resolveTenant"];
}
...
/**
 * Local dev convenience: `AUDIT_STORE=memory` runs the service against an
 * in-memory store so the gateway -> audit path works without a Postgres
 * database. Production leaves this unset.
 */
const useMemoryStore = process.env.AUDIT_STORE === "memory";
...
async function start(): Promise<void> {
  try {
    ...
    const store = useMemoryStore ? new MemoryAuditStore() : undefined;
