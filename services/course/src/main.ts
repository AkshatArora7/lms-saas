/**
 * course service.
 * Course CRUD, templates, sections, terms, course copy (offloaded to background
 * jobs). Sits behind the gateway, which authenticates requests and forwards the
...
import { registerCourseRoutes, type CourseRouteDeps } from "./routes.js";
...
export interface BuildAppOptions {
  config?: AppConfig;
  store?: CourseRouteDeps["store"];
  resolveTenant?: CourseRouteDeps["resolveTenant"];
}
...
/**
 * Local dev convenience: `COURSE_STORE=memory` runs the service against an
 * in-memory store seeded with demo courses so the gateway -> course path works
 * end-to-end without a Postgres database. Production leaves this unset.
 */
const useMemoryStore = process.env.COURSE_STORE === "memory";
