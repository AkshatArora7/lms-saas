/**
 * discussion service.
 * Forums, topics/threads, threaded posts and replies, pin/delete moderation and
 * graded participation. Emits discussion.post.created / discussion.topic.created
 * (subscriber fanout handled by the notification service). Sits behind the
 * gateway, which authenticates requests and forwards the resolved tenant as
 * `x-tenant-id`.
 *
...
/**
 * Local dev convenience: `DISCUSSION_STORE=memory` runs the service against an
 * in-memory store seeded with a demo forum/topic so the gateway -> discussion
 * path works end-to-end without a Postgres database. Production leaves this
 * unset.
 */
const useMemoryStore = process.env.DISCUSSION_STORE === "memory";
