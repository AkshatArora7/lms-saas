/**
 * enrollment service.
 * Enrollments, per-section role assignments, lifecycle (active/withdrawn/
 * completed). Maps OneRoster enrollments; owns the enroll->reserve-seat->confirm
 * saga. Sits behind the gateway, which authenticates requests and forwards the
 * resolved tenant as `x-tenant-id`. Bursty at term start (queue-depth scaling).
 *
...
/**
 * Local dev convenience: `ENROLLMENT_STORE=memory` runs the service against an
 * in-memory store seeded with a demo enrollment so the gateway -> enrollment
 * path works end-to-end without a Postgres database. Production leaves this
 * unset.
 */
const useMemoryStore = process.env.ENROLLMENT_STORE === "memory";
