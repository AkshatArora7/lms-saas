/**
 * grading service.
 * Gradebook: weighted categories, line items, grade schemes, calculated and
 * final grades. Serves OneRoster results + LTI AGS line items. Source of truth
 * for grades. Sits behind the gateway, which authenticates requests and forwards
 * the resolved tenant as `x-tenant-id`.
 *
...
/**
 * Local dev convenience: `GRADING_STORE=memory` runs the service against an
 * in-memory store seeded with a demo course gradebook so the gateway -> grading
 * path works end-to-end without a Postgres database. Production leaves this
 * unset.
 */
const useMemoryStore = process.env.GRADING_STORE === "memory";
