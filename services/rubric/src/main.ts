/**
 * rubric service.
 * Rubrics (criteria x levels), competencies, learning objectives and
 * activity<->outcome alignment. Sits behind the gateway, which authenticates
 * requests and forwards the resolved tenant as `x-tenant-id`. Feeds grading and
 * analytics.
 *
...
import { registerRubricRoutes, type RubricRouteDeps } from "./routes.js";
...
export interface BuildAppOptions {
  config?: AppConfig;
  store?: RubricRouteDeps["store"];
  resolveTenant?: RubricRouteDeps["resolveTenant"];
}
...
/**
 * Local dev convenience: `RUBRIC_STORE=memory` runs the service against an
 * in-memory store so the gateway -> rubric path works without a Postgres
 * database. Production leaves this unset.
 */
const useMemoryStore = process.env.RUBRIC_STORE === "memory";
...
async function start(): Promise<void> {
  try {
    ...
    const store = useMemoryStore ? new MemoryRubricStore() : undefined;
